...
import TipoRepository from "../repositories/tipoRepository.js";

export default class TipoController {

    async listar(req, res) {
        try {
            let repo = new TipoRepository();
            let lista = await repo.listar();
            if (lista != null && lista != "") {
                res.status(200).json(lista);
            } else {
                res.status(404).json({ msg: "Nenhum tipo cadastrado!" });
            }
        } catch (ex) {
    ...
    async gravar(req, res) {
        try {
            ...
            if (descricao) {   
                ...
                let entidade = new TipoEntity(0, descricao);
                let repo = new TipoRepository();
                let result = await repo.gravar(entidade);

                if (result) {
                    res.status(201).json({ msg: "Tipo cadastrado com sucesso!" });
                } else {
                    throw new Error("Erro ao inserir tipo no banco de dados");
                }
            } else {
    ...
    async alterar(req, res) {
        try {
            ...
            if (id && descricao) {
                let entidade = new TipoEntity(id, descricao);
                let repo = new TipoRepository();
                ...
                if (await repo.obter(id)) {
                    ...
                    res.status(404).json({ msg: "Tipo não encontrado para alteração" });
                }
            } else {
    ...
    try {
        ...
        if (await repo.obter(id)) {
            ...
            if (result) {
                res.status(200).json({ msg: "Tipo deletado com sucesso!" });
            } else {
                throw new Error("Erro ao executar a deleção no banco de dados");
            }
        } else {
            res.status(404).json({ msg: "Tipo não encontrado para deleção" });
        }
    } catch (ex) {
        if (ex.code === "ER_ROW_IS_REFERENCED_2" || ex.errno === 1451) {
            res.status(400).json({
                msg: "Não é possível excluir este tipo porque há registros vinculados a ele."
            });
        } else {
            res.status(500).json({ msg: ex.message });
        }
